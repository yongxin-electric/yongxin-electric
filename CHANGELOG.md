...
- 商品頁更新實物照片狀態、3a3b 接點、黃標 AC220V 線圈辨識與照片數量；未直接標示的容量、功耗與動作資料維持不補猜。
- 同步更新 `category-fuji-industrial-relays.html` 商品卡與 `assets/product-search-index.js` 搜尋資料。

## v7.21.18 (2026-09-14)
- 修正 `brands.html` 富士電機品牌卡的舊數量：由「查看 3 個商品」改為實際「查看 24 個商品」。
- 富士品牌頁目前為 24 個商品頁、4 個系列分類；4 代表系列分類數，不是商品頁數。
- 逐一核對常見品牌頁中 16 個已有商品品牌卡與其品牌／分類頁的實際商品頁數；除富士電機外，其餘 15 個品牌數量皆一致，無需修改。
...
- `category-fuji-contactors.html` 與 `assets/product-search-index.js` 同步更新上述商品的正面封面圖。
- `PROJECT_RULES.md` 新增商品照片數量規則：商品相簿無 5 張上限；使用者提供且確認可上站的照片原則上全部納入，只排除重複、失焦、供應鏈／隱私風險或使用權不明照片。
- `README.md` 版本號同步更新為 v7.21.13。

## v7.21.12 (2026-09-13)
- 全站巡檢 36 個分類頁；其中 26 個分類頁含商品卡，共檢查 249 個商品卡圖片實例（248 個獨立圖片來源），所有商品卡圖片檔案均可正常讀取。
- 分類卡圖片 CSS 維持 `object-fit: contain`，未發現因 `cover` 造成的截圖；針對 SC-0、SC-N1、SC-N2 AC110V、SC-N2 AC220V 參考圖內部留白過大問題，重新裁除空白並放大商品主體，分類卡與商品頁主圖同步改善。
- `SC-N5A AC220V` 恢復 v7.21.9 使用者原始實拍主圖，避免使用該商品未明確授權採用的外部參考圖。
- 巡檢 247 個商品頁：同頁「完整型號／產品型號／型號」未發現互相衝突；SEO `<title>` 均可對應產品識別；商品搜尋索引 247 筆標題與商品頁 H1 全數一致。
- 修正 24 個商品頁 Breadcrumb 最後一段未含商品型號的問題：2 頁 Mitsubishi NV100-KC、6 頁 Panasonic TB、15 頁 Shihlin BH、1 頁 Shihlin BLU。
- 新增 `SITE_AUDIT_v7_21_12.md`，記錄本次分類卡圖片與商品頁一致性稽核結果。

## v7.21.11 (2026-09-13)
- 檢查 Fuji 13 個接觸器商品頁的分類卡封面，並將 `category-fuji-contactors.html` 與 `assets/product-search-index.js` 同步為各商品頁目前實際主圖；其中 FMC-0 分類卡改為正面優先，`SRCa3631-2` 的分類卡與搜尋標題同步更新為「白頭 舊型」。
- 批次檢查多張商品照片頁面後，額外修正 7 個士林電機接觸器商品頁主圖角度：`S-P30T`、`S-P40T`、`S-P50T`、`S-P60T`、`S-P80T`、`S-P100T`、`S-P150T`，將商品詳情頁主圖與縮圖順序改為正面照優先。
- `assets/product-search-index.js` 的對應搜尋卡封面一併同步，避免站內搜尋結果仍顯示舊角度照片。
- `README.md` 版本號同步更新為 v7.21.11。

## v7.21.10 (2026-09-13)
- 修正 `assets/script.js` 商品相簿切換邏輯，兼容 `productMainImage` 與 `mainProductImage`，恢復全站商品頁第二張、第三張與其餘縮圖的點擊切換功能。
- 依使用者指定調整 13 個 Fuji Electric 接觸器商品頁封面：FMC-0、SC-0、SC-1N、SC-N1、SC-N2（AC110V／AC220V）、SC-N2S、SC-N4、SC-N5A（AC110V／AC220V）、SC-N7（兩頁）、SRCa3631-2。
- 補齊 `SC-0 AC220V`、`SC-N1 AC220V｜NEO SC`、`SC-N2 AC110V`、`SC-N2 AC220V` 的商品圖庫，各頁現可切換 3 張商品圖。
- 既有多張實拍頁面的封面照片改為正面優先；`SRCa3631-2 AC220V` 頁面標題更新為「SRCa3631-2 AC220V｜白頭 舊型」。
- `README.md` 版本號同步更新為 v7.21.10。
...
## v7.21.5 (2026-09-09)
- 新增世僖科技 SHIH SHIN SH-AS 系列 AC 疊片型電磁鐵／電吸鐵 15 個實拍規格頁，商品總數由 209 增加至 224。
- 新增 `category-shih-shin-solenoids.html` 與 `brand-shih-shin.html`，同步更新自動控制元件、常見品牌、首頁品牌入口與商品搜尋。
- 本批電吸鐵規格依實物銘牌與世僖科技原廠 AC Laminated Solenoid 規格表交叉核對，包含 AC110／220V、50／60Hz、行程、出力、尺寸與重量。
- 新增「電磁元件線圈／驅動架構查證 SOP」與「電吸鐵／Solenoid 規格查證 SOP」。
- 既有士林 S-P／MSO-P、TECO 接觸器、IDEC RR／RU、OMRON MY／LY／MK 等具線圈商品頁完成線圈／驅動架構欄位回補，共 61 頁。
- 商品圖片均使用實物照片與既定永信低干擾浮水印；未使用外部商品圖片。


## v7.21.4 (2026-09-04)
- 新增服務實績：特殊軍規型接頭客製連接線重製。
- 客戶原線因外力受壓損壞，依原件重新製作；未確認的軍規系列、MIL 規範、針數與腳位不自行推定。
- 新增 6 張實際製作照片並套用既定永信浮水印。
- `case-studies.html`、`category-custom-special.html`、`services.html` 新增案例入口。
- 預留未來軍規接頭商品頁與服務案例雙向連結。

## v7.21.3
- 新增 Panasonic 國際牌 TB 系列 6 個定時開關商品頁與品牌／分類頁。
- 依完整型號區分電源、AC／Quartz 驅動、停電補償與 1a／1c 接點；TB39909NT7S3 保持獨立型號。
- 商品搜尋由 203 增加至 209 個商品頁。

## v7.21.2
- 新增士林電機 BLU 2P 50A 無熔絲斷路器商品頁與 BLU 系列分類。

## v7.21.1
- 實品商品照片禁止標示「商品示意／示意照」。

## v7.21.0
- 新增三菱電機 NV100-KC 3P 75A／100A 漏電斷路器商品頁、分類與品牌頁。
- 建立商品頁與技術專欄的雙向關聯規則。

## v7.20.9
- 新增三菱 NV100 漏電斷路器送電冒火花跳脫技術文章。

## v7.20.8
- 新增 DC 24V 控制電源不穩技術文章；前台取消「每日一則」名稱，統一使用「技術專欄」。

## v7.20.7
- KSS CV-200M 彩色總覽補入黃色 CV-200MYW。

## v7.20.5
- 新增 KSS CV-200M 彩色特殊訂製系列，整理 9 個彩色完整型號。
